...
PROJECT_ROOT_PATH = os.path.realpath(os.path.dirname(os.path.dirname(__file__)))
...
CC_COMMENT = r'(?:^(?:\s*(?://.*)?(?:/\*(?:.|\n)*?\*/)?)$\n)*'
CC_HEADER_PREAMBLE = re.compile(CC_COMMENT + r'(?:^#ifndef (.*?)$\n#define \1$)?', re.M)
...
def to_project_path(path: str):
  if not path.startswith(PROJECT_ROOT_PATH):
    raise GenerateError('Path "{}" does not belong to project.'.format(path))
  return os.path.relpath(path, PROJECT_ROOT_PATH)
...
def generate_pair_command(name, path, force, **_):
  ...
  print('Generating header "{}"'.format(header))
  if os.path.exists(header) and not force:
    raise GenerateError('Header file already exists. Use --force to overwrite.')
  generate_header(header)

  print('Generating source "{}"'.format(source))
  if os.path.exists(source) and not force:
    raise GenerateError('Source file already exists. Use --force to overwrite.')
    ...
    if update_frame:
      ...
      if path.endswith(HEADER_EXT) or path.endswith(SOURCE_EXT):
        ...
        if path.endswith(HEADER_EXT):
          ok = text.startswith(header_preamble(path)) and text.endswith(header_epilogue(path))
          if not ok:
            print('Frame is invalid for header')
            text = CC_HEADER_PREAMBLE.sub('', text, 1)
            text = CC_HEADER_EPILOGUE.sub('', text, 1).strip() + '\n\n'
...
def parse_args():
  ...
  sub = parser.add_subparsers(description='Perform specific generation/update task')

  header = sub.add_parser('header', parents=(base,), description='Generates single header file.')
  header.add_argument('name', help='Header file name (without extension).')
  header.set_defaults(func=generate_header_command)

  pair = sub.add_parser('pair', parents=(base,), description='Generates header/source file pair.')
  ...
  update.add_argument('paths', nargs='+', help='Project file path to work with.')
