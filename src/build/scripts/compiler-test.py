...
def parse_args():
    ...
    compile_parser = subparsers.add_parser('clang++')
    compile_parser.add_argument('-c', dest='source', required=True)
    compile_parser.add_argument('-o', dest='output', required=True)
    compile_parser.add_argument('-MF', dest='depfile', required=True)
    compile_parser.set_defaults(tag='compile')

    link_parser = subparsers.add_parser('link')
    link_parser.add_argument('inputs', nargs='+')
    link_parser.add_argument('-o', dest='output', required=True)
    link_parser.set_defaults(tag='link')
...
class TestHeaderParser:
    ...
    def parse_test(self, word: str):
        if word != self.TEST_BEGIN:
            raise SourceParseException(
                '"{}" expected.'.format(self.TEST_BEGIN))
...
class SourceParser:
    ...
    def global_header(self, line: str):
        if line != self.GLOBAL_HEADER:
            raise SourceParseException(
                'First compiler test line must be "{}"'.format(self.GLOBAL_HEADER))
...
def compile(args, rest_args):
    ...
    with open(args.output, 'w') as out:
        out.write(TEST_SCRIPT.format(cmdline=['clang++'] + rest_args,
